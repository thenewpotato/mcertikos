...
void smallfile(void)
{
    ...
    // Our custom test on file_stat. We manually confirmed that the size of the file is correct (reflected in terminal logging)
    struct file_stat st;
    if (fstat(fd, &st) != 0)
    {
...
void bigfile1(void)
{
    ...
    for (;;)
    {
        ...
        {
            printf("read failed %d after %d/%d blocks\n", i, n, MAXFILE);
            exit();
        }
        if (((int *)buf)[0] != n)
        {
